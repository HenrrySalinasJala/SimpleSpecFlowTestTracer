namespace NUnit.Engine.Listeners
{
    ...
    public static class TestUtil
    {
        ...
        public static XmlNode CreateTestCaseSkipped(string id, string parentId, string name, string duration, string message)
        {
            return CreateMessage(string.Format("<test-case id=\"{0}\" {1} name=\"{2}\" fullname=\"{2}\" runstate=\"Runnable\" result=\"Skipped\" duration=\"{3}\" asserts=\"0\"><property name=\"_SKIPREASON\" value=\"{4}\" /><reason><message><![CDATA[{4}]]></message></reason></test-case>", id, GetNamedAttr("parentId", parentId), name, duration, message));
        }

        public static XmlNode CreateTestCaseInconclusive(string id, string parentId, string name, string duration)
        {
            return CreateMessage(string.Format("<test-case id=\"{0}\" {1} name=\"{2}\" fullname=\"{2}\" runstate=\"Runnable\" result=\"Inconclusive\" duration=\"{3}\" asserts=\"0\"></test-case>", id, GetNamedAttr("parentId", parentId), name, duration));
        }
