namespace NUnit.Engine.Listeners
{
    using System;
    using System.Text.RegularExpressions;

    public class SimpleListenerVersion: IComparable<SimpleListenerVersion>
    {
        private static readonly Regex VersionRegex = new Regex("^(\\d+)(\\.(\\d+)|).*", RegexOptions.Compiled);

        public SimpleListenerVersion(string version)
        {
            try
            {
                if (!string.IsNullOrEmpty(version))
                {
                    var match = VersionRegex.Match(version);
                    if (match.Success)
                    {
                        int val;
                        if (int.TryParse(match.Groups[1].Value, out val))
                        {
                            Major = val;
                        }

                        if (int.TryParse(match.Groups[3].Value, out val))
                        {
                            Minor = val;
                        }
                    }
                }
            }
            catch
            {
                // ignored
            }
        }

        public int Major { get; private set; }

        public int Minor { get; private set; }

        public int CompareTo(SimpleListenerVersion other)
        {
            var result = Major.CompareTo(other.Major);
            if (result != 0)
            {
                return result;
            }

            return Minor.CompareTo(other.Minor);
        }

        public override string ToString()
        {
            return Major + "." + Minor;
        }
    }
}
